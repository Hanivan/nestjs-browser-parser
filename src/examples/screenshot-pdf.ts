...
async function demonstrateScreenshotPdf() {
  ...
  try {
    ...
    console.log(`📸 Basic screenshot captured: ${basicScreenshot.length} bytes`);

    // Example 2: Full page screenshot
    console.log('\n2. Full Page Screenshot');
    console.log('=======================');

    const fullPageScreenshot = await parser.takeScreenshot('https://example.com', {
      type: 'png',
      fullPage: true,
      timeout: 10000,
    });
    ...
    const customViewportScreenshot = await parser.takeScreenshot('https://example.com', {
      type: 'png',
      viewport: { width: 1024, height: 768 },
      timeout: 10000,
    });

    console.log(`📸 Custom viewport (1024x768): ${customViewportScreenshot.length} bytes`);

    // Example 4: Mobile viewport screenshot
    console.log('\n4. Mobile Viewport Screenshot');
    console.log('==============================');

    const mobileScreenshot = await parser.takeScreenshot('https://example.com', {
      type: 'png',
    ...
    console.log(`📱 Mobile viewport (375x812): ${mobileScreenshot.length} bytes`);
    ...
    console.log(`📸 JPEG screenshot (quality 80): ${jpegScreenshot.length} bytes`);

    // Example 6: Clipped screenshot
    console.log('\n6. Clipped Screenshot');
    console.log('=====================');

    const clippedScreenshot = await parser.takeScreenshot('https://example.com', {
      type: 'png',
      clip: {
        x: 0,
        y: 0,
    ...
    console.log(`✂️ Clipped screenshot (800x600): ${clippedScreenshot.length} bytes`);
    ...
    const advancedScreenshot = await parser.takeScreenshot('https://httpbin.org/html', {
      type: 'png',
      fullPage: true,
    ...
    try {
      ...
      const screenshot = await parser.takeScreenshot('https://httpbin.org/html', {
        type: 'png',
        fullPage: true,
      });
      ...
      const pageInfo = parser.extractStructuredFromHtml<PageInfo>(response.html, {
